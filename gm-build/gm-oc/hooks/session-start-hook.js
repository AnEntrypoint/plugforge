...
try {
  ...
  outputs.push('Use the Skill tool with skill: "gm" to begin — do NOT use the Agent tool to load skills. Skills are invoked via the Skill tool only, never as agents. All code execution uses exec:<lang> via the Bash tool — never direct Bash(node ...) or Bash(npm ...) or Bash(npx ...). Browser CLI commands use agent-browser: prefix (not exec:agent-browser). exec:agent-browser is for JS eval in the browser only.');
