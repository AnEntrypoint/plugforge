...
**rs-plugkit CI builds**: Internet is slow — never run `cargo update` or `cargo build` locally. Instead, push code + Cargo.toml changes and let GitHub Actions build. If Cargo.lock needs updating, use `gh` to trigger a workflow or push a no-op commit. The CI release workflow builds on tag push (`v*`).
