function buildHooksMapCustom(hookEventNames, hookOutputFormat, createCustomCommand) {
  const hooks = {};
  // See template-builder.buildHooksMap for rationale on these values.
  const hookDefs = [
    { event: 'preTool', hookEvent: 'pre-tool-use', timeout: 15000 },
    ...
    { event: 'stop', hookEvents: [
      { hookEvent: 'stop', timeout: 30000 },
      { hookEvent: 'stop-git', timeout: 210000 }
    ]}
  ];
  hookDefs.forEach(def => {
    ...
    if (def.hookEvents) {
      ...
      const cmd = createCustomCommand(def.hookEvent, def.timeout);
