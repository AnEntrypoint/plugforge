...
class CLIAdapter extends PlatformAdapter {
  ...
  createFileStructure(pluginSpec, sourceDir) {
    ...
    const structure = {
      ...
      'pre-tool-use.js': readFile(this.getHookSourcePaths('pre-tool-use')),
      'session-start.js': readFile(this.getHookSourcePaths('session-start')),
      'prompt-submit.js': readFile(this.getHookSourcePaths('prompt-submit')),
      'stop.js': readFile(this.getHookSourcePaths('stop')),
      'stop-git.js': readFile(this.getHookSourcePaths('stop-git')),
  ...
  getHookSourcePaths(hook) {
    const canonical = {
      'pre-tool-use': 'hooks/pre-tool-use.js',
      'session-start': 'hooks/session-start.js',
      'prompt-submit': 'hooks/prompt-submit.js',
      'stop': 'hooks/stop.js',
      'stop-git': 'hooks/stop-git.js'
    };
    return [canonical[hook] || `hooks/${hook}.js`];
  }
