...
const PLATFORM_META = {
  ...
  vscode: { repoId: 'gm-vscode', label: 'VS Code', type: 'ide', badgeLabel: 'vscode', useWasm: true, installSteps: [{ desc: 'Extensions → search "gm" → install' }, { desc: 'reload VS Code' }] },
  cursor: { repoId: 'gm-cursor', label: 'Cursor', type: 'ide', badgeLabel: 'cursor', useWasm: true, installSteps: [{ desc: 'Extensions → search "gm" → install' }, { desc: 'reload Cursor' }] },
  zed: { repoId: 'gm-zed', label: 'Zed', type: 'ide', badgeLabel: 'zed', useWasm: true, installSteps: [{ desc: 'build from source', cmd: 'git clone https://github.com/AnEntrypoint/gm-zed && cd gm-zed && cargo build --release' }, { desc: 'copy to extensions dir', cmd: 'cp target/release/libgm.so ~/.config/zed/extensions/gm/' }, { desc: 'restart Zed' }] },
  jetbrains: { repoId: 'gm-jetbrains', label: 'JetBrains', type: 'ide', badgeLabel: 'jetbrains', useWasm: true, installSteps: [{ desc: 'Preferences → Plugins → search "gm" → install' }, { desc: 'restart IDE' }] },
  ...
  antigravity: { repoId: 'gm-antigravity', label: 'Antigravity', type: 'ide', badgeLabel: 'antigravity', useWasm: true, installSteps: [{ desc: 'install via Antigravity CLI', cmd: 'antigravity --install-extension gm.gm-antigravity' }, { desc: 'or install via npm', cmd: 'npm install -g gm-antigravity' }, { desc: 'reload Antigravity' }] },
  windsurf: { repoId: 'gm-windsurf', label: 'Windsurf', type: 'ide', badgeLabel: 'windsurf', useWasm: true, installSteps: [{ desc: 'Extensions -> search "gm" -> install' }, { desc: 'reload Windsurf' }] },
  thebird: { repoId: 'gm-thebird', label: 'thebird (browser via Freddie + plugkit.wasm)', type: 'browser', badgeLabel: 'thebird', useWasm: true, installSteps: [{ desc: 'install via npm', cmd: 'npm install -g gm-thebird' }, { desc: 'run installer', cmd: 'gm-thebird' }, { desc: 'restart Freddie' }] }
};
