...
const PLATFORMS = {
  ...
  'kilo': () => createAdapterClass(cliConfig[4]),
  'qwen': () => createAdapterClass(cliConfig[5]),
  'hermes': () => createAdapterClass(cliConfig[6]),
  'thebird': () => createAdapterClass(cliConfig[7]),
  ...
  'antigravity': () => require('../platforms/antigravity'),
  'windsurf': () => require('../platforms/windsurf')
};
...
class AutoGenerator {
  ...
  async generate() {
    ...
    const sharedSkills = TemplateBuilder.loadSharedSkills(this.pluginDir);

    for (const [platformName, AdapterFactory] of Object.entries(PLATFORMS)) {
      try {
        const platformDir = path.join(this.outputDir, `gm-${platformName}`);
        const AdapterClass = AdapterFactory();
        const config = TemplateBuilder.selectBundledSkills(platformName) ? { skillsCache: sharedSkills } : {};
  ...
  validateGeneratedFiles() {
    ...
    const platformRequirements = {
      ...
      'antigravity': { hasPackageJson: true, hasReadme: true, hasAgents: true },
      'windsurf': { hasPackageJson: true, hasReadme: true, hasAgents: true }
    };
  ...
  logResults() {
    ...
    if (successfulPlatforms.length > 0) {
      ...
      successfulPlatforms.forEach(platform => {
        const platformDir = path.join(this.outputDir, `gm-${platform}`);
