...
class AutoGenerator {
  ...
  validateGeneratedFiles() {
    ...
    Object.keys(PLATFORMS).forEach(platform => {
      ...
      const actualFiles = new Set(this.getPlatformFiles(platformDir));
      validation[platform] = {
        status: 'success',
        fileCount: actualFiles.size,
        hasPackageJson: actualFiles.has('package.json'),
        hasReadme: actualFiles.has('README.md'),
        hasAgents: Array.from(actualFiles).some(f => f.startsWith('agents/')),
        hasSkills: Array.from(actualFiles).some(f => f.startsWith('skills/'))
      };
    });
    return validation;
  }
