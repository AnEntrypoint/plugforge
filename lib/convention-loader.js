...
class ConventionLoader {
  static load(pluginDir) {
    ...
    const gmJsonPath = path.join(resolvedDir, 'gm.json');
    if (!fs.existsSync(gmJsonPath)) {
      throw new Error(`gm.json not found in ${resolvedDir}`);
    }

    const gmJson = JSON.parse(fs.readFileSync(gmJsonPath, 'utf-8'));
  ...
  static validate(loaded) {
    ...
    if (!loaded.spec.name) errors.push('gm.json must have name');
    if (!loaded.spec.version) errors.push('gm.json must have version');
    if (!loaded.spec.author) errors.push('gm.json must have author');
    if (!loaded.spec.license) errors.push('gm.json must have license');
