          ...
          eventHooks.forEach(hookGroup => {
            if (hookGroup.hooks && Array.isArray(hookGroup.hooks)) {
              hookGroup.hooks.forEach(hook => {
                if (hook.command && (hook.command.includes('${__dirname}') || hook.command.match(/\$\{[a-zA-Z_]+\}/))) {
                  const match = hook.command.match(/(?:\$\{[a-zA-Z_]+\}|(?:\$\{__dirname\}))\/(?:hooks\/|bin\/)?(.*?)(?:\s|$)/);
                  if (match) {
                ...
                } else if (hook.command && !hook.command.match(/\$\{[a-zA-Z_]*__dirname[a-zA-Z_]*\}/) && !hook.command.match(/\$\{[a-zA-Z_]+\}/)) {
