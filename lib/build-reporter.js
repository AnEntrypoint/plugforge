          ...
          eventHooks.forEach(hookGroup => {
            if (hookGroup.hooks && Array.isArray(hookGroup.hooks)) {
              hookGroup.hooks.forEach(hook => {
                // Verify path pattern is used (either ${__dirname} or environment variables)
                if (hook.command && (hook.command.includes('${__dirname}') || hook.command.match(/\$\{[A-Z_]+\}/))) {
                  // Extract filename from command
                  const match = hook.command.match(/(?:\$\{[A-Z_]+\}|(?:\$\{__dirname\}))\/(?:hooks\/)?(.*?)(?:\s|$)/);
                  if (match) {
                    const hookFile = match[1];
                    const hookPath = path.join(hooksDir, hookFile);
                    if (!fs.existsSync(hookPath)) {
                      issues.push(`❌ Referenced hook file missing: ${hookFile}`);
                    }
                  }
