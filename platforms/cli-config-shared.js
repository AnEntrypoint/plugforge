...
function pluginMjsSource(pluginFile) {
  const lines = [
    ...
    "        throw new Error('The Explore agent is blocked. Use exec:codesearch in the Bash tool instead.\\n\\nexec:codesearch\\n<natural language description of what to find>\\n\\nFor raw JIT execution, write code to `.gm/exec-spool/in/<task>.json` and let the spool watcher execute it.');",
    "      }",
    "      if (input.tool === 'Skill') {",
    ...
    "      const m = cmd.match(/^exec(?::(\\S+))?\\n([\\s\\S]+)$/);",
     "      if (!m) {",
     "        throw new Error('Use exec: prefix for Bash. The command must start with `exec` or `exec:<lang>` on its own line, then the body on the next line. Examples:\\n\\nexec\\nls -la\\n\\nexec:nodejs\\nconsole.log(\"hello\")\\n\\nexec:bash\\ngit status\\n\\nLanguages: nodejs (default), bash, python, typescript, go, rust, deno, cmd. File I/O via exec:nodejs + require(\"fs\"). Raw JIT execution can also use `.gm/exec-spool/in/<task>.json`; the spool watcher executes it and writes `.gm/exec-spool/out/<task>.json`. Codebase search: exec:codesearch on its own line, then a two-word query.');",
     "      }",
     "      const rawLang = (m[1]||'').toLowerCase();",
     "      if (rawLang === 'bash' || rawLang === 'sh' || rawLang === '') {",
     "        const banned = bashBannedTool(m[2]);",
    "        if (banned) throw new Error('`'+banned+'` is blocked in exec:bash. Use exec:codesearch instead. For raw JIT execution, write code to `.gm/exec-spool/in/<task>.json` and let the spool watcher execute it.');",
     "      }",
