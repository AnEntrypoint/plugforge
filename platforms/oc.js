...
class OpenCodeAdapter extends CLIAdapter {
  constructor() {
    super({
      name: 'oc',
      label: 'OpenCode',
  ...
  getAdditionalFiles(pluginSpec, readFile) {
    return {
      ...
      'setup.sh': this.generateSetupSh(),
      'setup.bat': this.generateSetupBat(),
      'setup-global.js': readFile(['glootie-oc/setup-global.js']),
      'install-global.sh': this.generateInstallGlobalSh(),
      'install-global.bat': this.generateInstallGlobalBat()
    };
  }
  ...
  buildHooksMap(pluginSpec) {
    return {
      'session.created': {
        handler: 'runSessionStartHook',
        description: 'Initialize session with thorns analysis and start.md'
      },
      'session.idle': {
        handler: 'runStopHook',
        description: 'Verify git state and run eval scripts before session end'
      }
    };
  }

  generateReadme() {
    return `# gm Plugin for OpenCode
