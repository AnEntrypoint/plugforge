...
const run = () => {
  ...
  try {
    ...
    // Check if plan.md is empty or non-existent
    if (!fs.existsSync(planFile)) {
      // Plan file doesn't exist, allow stop to proceed
      return { decision: undefined };
    }

    const planContent = fs.readFileSync(planFile, 'utf-8').trim();
    const hasItems = planContent.split('\n').some(line => {
      const trimmed = line.trim();
      return trimmed && !trimmed.startsWith('#');
    });

    if (!hasItems) {
      // Plan is empty, allow stop to proceed
      return { decision: undefined };
    }

    // Plan has items remaining
    let instruction = `The plan.md file still has items to complete. Continue working through the plan:\n\n${planContent}\n\nAfter completing each item, remove it from plan.md. Once all items are completed and plan.md is empty or deleted, the session can end.`;
