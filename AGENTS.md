...
**Shared memory & search index are tracked, never ignored**: `.gm/rs-learn.db` (rs-learn episodes / community summaries) and `.gm/code-search/` (rs-codeinsight chunk index, relocated from legacy top-level `.code-search/`) are intentionally committed so memory and index state shares across machines, sessions, and CI. Tooling, scripts, and any agent editing `.gitignore` must NEVER add `.gm/rs-learn.db`, `.gm/`, `.gm/code-search/`, or legacy `.code-search/` to ignore rules. The `.gm/` parent cannot be ignored as a whole — per the gitignore parent-re-include caveat, individual `.gm/*` entries (prd-state.json, lastskill, turn-state.json, trajectory-drafts/, ingest-drafts/, rslearn-counter.json, etc.) are listed one-by-one between `# >>> gm managed` markers, leaving `.gm/rs-learn.db` and `.gm/code-search/` un-ignored. Same rule applies to downstream repos: `lib/template-builder.js::generateGitignore()` must not emit `.gm/`, `.gm/rs-learn.db`, `.gm/code-search/`, or `.code-search/` lines. Hard rule for new tooling: any project-local persistent state (chunk index, DB, embeddings) must write under `.gm/<name>/`, never to a top-level dotfile/dotdir. rs-search write sites (`src/main.rs`, `src/scanner.rs`) use `root.join(".gm").join("code-search")` with legacy `.code-search/` auto-rename on first run; `src/ignore.rs` IGNORE_DIRS includes `.gm` so the scanner doesn't recurse into the index dir while building it. rs-plugkit `doctor` checks new path with legacy fallback. If a session writes a stray `rs-learn.db` at repo root (legacy migration path), move it to `.gm/rs-learn.db` rather than gitignoring it.

**rs-exec runtime must NOT mutate `.gitignore` for shared-state paths**: `rs-exec/src/runtime.rs::PLUGKIT_IGNORE_PATTERNS` + `ensure_plugkit_gitignore` runs on every init and may only list genuinely ephemeral local state: `.plugkit-browser-profile/`, `.plugkit-browser-profile-*/`, `.plugkit-agent-worktree/`, `*.stackdump`. Adding `.code-search/`, `.gm/code-search/`, `.codeinsight`, `.gm/rs-learn.db`, or any shared-tracked path here breaks the cross-machine memory/index contract — symptom is recurring `.gitignore` dirty-state with stray `+.code-search/` line every gm turn. When investigating recurring `.gitignore` dirt, audit every Rust hook crate's `ensure_*gitignore` helpers and any session_start / runtime init code that mutates `.gitignore`; fix the source crate, never patch with reverts.
...
**rs-plugkit hook sync shims must not nest tokio runtimes**: `main()` is `#[tokio::main]` (outer multi-thread runtime). Sync `pub fn`s in `src/hook/rs_learn.rs` that wrap async rs-learn calls must NOT use a private `OnceLock<Runtime>` + `rt().block_on()` — that path panics "Cannot start a runtime from within a runtime" at tokio `scheduler/multi_thread/mod.rs:91:9` whenever the helper is invoked from inside async main. Symptom: `plugkit recall`, `plugkit learn status`, `plugkit memorize`, `plugkit forget` all panic; `doctor`/`search`/`codeinsight`/`runner` unaffected. Fix: use `Handle::try_current()` to detect ambient runtime, then `tokio::task::block_in_place(|| handle.block_on(work))`; only fall back to a private runtime when no ambient runtime exists. Same pattern for `spawn_detached` — prefer `handle.spawn` over `rt().spawn` when ambient. General rule: any sync `pub fn` shim wrapping `block_on` that may be called from inside an async runtime needs the `Handle::try_current()` branch.
